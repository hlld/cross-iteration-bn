...
def network(inputs, use_bn=True, use_cbn=True, is_training=False):
    ...
    if use_bn:
        weight = tf.get_default_graph().get_tensor_by_name('conv2d_1/kernel:0')
        net = BatchNormalization(use_cbn=use_cbn)(net, weight, is_training)
    net = tf.nn.relu(net)
    net = utils.conv2d(net, 24, kernel_size=3, strides=1, use_bias=use_bias)
    if use_bn:
        weight = tf.get_default_graph().get_tensor_by_name('conv2d_2/kernel:0')
        net = BatchNormalization(use_cbn=use_cbn)(net, weight, is_training)
    net = tf.nn.relu(net)
    net = utils.conv2d(net, 32, kernel_size=3, strides=2, use_bias=use_bias)
    if use_bn:
        weight = tf.get_default_graph().get_tensor_by_name('conv2d_3/kernel:0')
        net = BatchNormalization(use_cbn=use_cbn)(net, weight, is_training)
    net = tf.nn.relu(net)
    net = utils.conv2d(net, 48, kernel_size=3, strides=1, use_bias=use_bias)
    if use_bn:
        weight = tf.get_default_graph().get_tensor_by_name('conv2d_4/kernel:0')
        net = BatchNormalization(use_cbn=use_cbn)(net, weight, is_training)
    net = tf.nn.relu(net)
